...
function driver(actor = "mira.santos", host = { pickFile: async () => null }) {
  return createDriver(
    {
...
function bootMessages(remote) {
  return remote.tick().map((message) => JSON.parse(message));
}

function projection(messages, port, name, key = null) {
...
function update(outcome, port, name, key = null) {
...
function command(port, name, payload, correlation = `${port}-${name}`) {
...
function onlyOutcome(messages) {
  const outcomes = messages.filter((message) => message.kind === "outcome");
  assert.equal(outcomes.length, 1);
  assert.deepEqual(outcomes[0].outcome, { ok: {} });
  return outcomes[0];
}
...
test("boot projects actor-filtered home, playable video, sequences, profiles, and explore", async () => {
  ...
  await withFetch(async (input, init) => {
    ...
    assert.equal(home.stories[0]["is-self"], true);
    assert.equal(home.stories[0]["has-unseen"], false);
    assert.deepEqual(home.posts[0].media, {
      video: {
        src: "video-lena",
        poster: {
          src: "poster-lena",
    ...
    assert.equal(home.posts[0]["viewer-has-liked"], true);
    assert.equal(home.posts[0]["viewer-has-saved"], false);
...
test("follow and unfollow refresh the relationship-filtered feed and story tray", async () => {
  ...
  await withFetch(async (input, init) => {
    ...
    for (const call of rpcCalls) {
      ...
      assert.deepEqual(JSON.parse(call.init.body), { target: THEO });
    }
  });
});
...
test("save and unsave settle every viewer-specific post surface and private grid", async () => {
  ...
  await withFetch(async (input, init) => {
    ...
    for (const call of rpcCalls) {
      assert.equal(new Headers(call.init.headers).get("x-spock-actor"), MIRA);
      assert.deepEqual(JSON.parse(call.init.body), { post: "post-lena-video" });
    }
  });
});
...
test("viewing one frame advances the ring and refreshes sequence progress", async () => {
  ...
  await withFetch(async (input, init) => {
    ...
    if (url.endsWith("/mark_story_viewed")) {
      const { story } = JSON.parse(init.body);
...
test("empty create metadata publishes and uses a provenance-only fallback alt", async () => {
  ...
  await withFetch(async (input, init) => {
    ...
    if (url.endsWith("/create_image_post")) {
      publishedPayload = JSON.parse(init.body);
      data.posts.push({
        id: "post-upload",
        author: { id: MIRA },
        caption: publishedPayload.caption,
        published_at: "2026-07-13T18:00:00Z",
        show_in_feed: true,
        media_kind: "image",
        media_file: { id: publishedPayload.image },
        video_file: null,
        media_alt: publishedPayload.alt,
      });
