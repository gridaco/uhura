// Generated from providers/spock.ts; do not edit.
//#region ../examples/instagram-uhura/providers/spock.ts
var PAGE_SIZE = 4;
...
function encode(value) {
	...
	if (encoded === void 0) throw new Error("provider tried to encode `undefined`");
	return encoded;
}
/**
* @param {unknown} error
...
function ageLabel(timestamp) {
	...
	if (!Number.isFinite(instant)) throw new Error(`invalid authority timestamp \`${timestamp}\``);
...
* @returns {SpockDriver}
*/
function createDriver({ graphql_url, rpc_url, storage_url, actor }, host) {
	const graphqlUrl = graphql_url.replace(/\/+$/, "");
	const rpcUrl = rpc_url.replace(/\/+$/, "");
	const storageUrl = storage_url.replace(/\/+$/, "");
	if (graphqlUrl.length === 0) throw new Error("Spock provider needs `graphql_url`");
	if (rpcUrl.length === 0) throw new Error("Spock provider needs `rpc_url`");
	if (storageUrl.length === 0) throw new Error("Spock provider needs `storage_url`");
	const whoamiUrl = new URL("/~whoami", graphqlUrl).toString();
	const outbox = [];
	let inflight = 0;
	let commandTail = Promise.resolve();
	const signedAssets = /* @__PURE__ */ new Map();
	const signingAssets = /* @__PURE__ */ new Map();
	const uploadedFileNames = /* @__PURE__ */ new Map();
	...
	* @returns {number}
	*/
	function mintRevision(port, projection, key) {
	...
	* @returns {string}
	*/
	function projectionMsg(port, projection, key, value) {
		return encode({
			kind: "projection",
			port,
			projection,
	...
	*/
	function projectionUpdate(port, projection, key, value) {
		return {
			port,
			projection,
			key: key ?? null,
	...
	*/
	async function fetchSnapshot() {
		...
		if (!response.ok) throw new Error(`POST /graphql/v1: ${response.status} ${body}`);
	...
	*/
	async function verifyViewer() {
		...
		const response = await fetch(whoamiUrl, { headers: { "x-spock-actor": expected } });
		const body = await response.text();
		if (!response.ok) throw new Error(`GET /~whoami: ${response.status} ${body}`);
		const identity = JSON.parse(body);
		if (identity.anonymous || !identity.known || identity.actor !== expected) throw new Error(`Spock did not recognize resolved actor \`${expected}\`: ${body}`);
	}
	/**
	* @param {string} fn
	...
	*/
	async function resolveAsset(asset) {
		...
		const signing = (async () => {
			const response = await fetch(`${storageUrl}/object/sign/${encodeURIComponent(asset)}`, {
				method: "POST",
				headers: { "x-spock-actor": viewerId() }
			});
			const body = await response.text();
			if (!response.ok) throw new Error(`POST /storage/v1/object/sign/${asset}: ${response.status} ${body}`);
			const envelope = JSON.parse(body);
			if (typeof envelope.url !== "string") throw new Error("Spock storage signing returned no URL");
			const absolute = new URL(envelope.url, storageUrl).toString();
			const expiry = Number(new URL(absolute).searchParams.get("exp"));
			const refreshAt = Number.isFinite(expiry) ? Math.max(Date.now(), expiry * 1e3 - 3e4) : Date.now();
			signedAssets.set(asset, {
				url: absolute,
				refreshAt
			});
			return absolute;
	...
	*/
	async function uploadFile(file) {
		...
		if (!SUPPORTED_IMAGE_TYPES.has(contentType)) throw new Error("Choose an image file (JPEG, PNG, or WebP)");
		const mintResponse = await fetch(`${storageUrl}/object/upload/sign`, {
			method: "POST",
			headers: { "x-spock-actor": viewerId() }
		});
		const mintBody = await mintResponse.text();
		if (!mintResponse.ok) throw new Error(`POST /storage/v1/object/upload/sign: ${mintResponse.status} ${mintBody}`);
		const mint = JSON.parse(mintBody);
		if (typeof mint.id !== "string" || typeof mint.url !== "string") throw new Error("Spock storage upload signing returned no object id or URL");
		...
		if (!putResponse.ok) throw new Error(`PUT signed storage object: ${putResponse.status} ${putBody}`);
	...
	const db = {
		...
		stories: [],
		storyViews: /* @__PURE__ */ new Set(),
		posts: [],
		slidesByPost: /* @__PURE__ */ new Map(),
		commentsByPost: /* @__PURE__ */ new Map(),
		likeCounts: /* @__PURE__ */ new Map(),
		liked: /* @__PURE__ */ new Set(),
		saved: /* @__PURE__ */ new Set(),
		follows: /* @__PURE__ */ new Set(),
		followersByUser: /* @__PURE__ */ new Map(),
		followingByUser: /* @__PURE__ */ new Map(),
		taggedPostsByUser: /* @__PURE__ */ new Map()
	};
	...
	*/
	async function loadAll() {
		...
		db.stories = data.stories.map((story) => ({
			id: story.id,
			...
			return newestFirst(left.published_at, right.published_at) || left.position - right.position || left.id.localeCompare(right.id);
		});
		db.storyViews = new Set(data.storyViews.map((view) => edgeKey(view.viewer.id, view.story.id)));
		db.posts = data.posts.map((post) => ({
			id: post.id,
			...
			return newestFirst(left.published_at, right.published_at) || left.id.localeCompare(right.id);
		});
		...
		for (const rows of db.slidesByPost.values()) rows.sort((left, right) => left.position - right.position);
		...
		for (const rows of db.commentsByPost.values()) rows.sort((left, right) => {
			return oldestFirst(left.created_at, right.created_at) || left.id.localeCompare(right.id);
		});
		db.likeCounts = /* @__PURE__ */ new Map();
		for (const like of data.likes) {
			const post = like.post.id;
			db.likeCounts.set(post, (db.likeCounts.get(post) ?? 0) + 1);
		}
		db.follows = /* @__PURE__ */ new Set();
		db.followersByUser = /* @__PURE__ */ new Map();
		db.followingByUser = /* @__PURE__ */ new Map();
		...
		db.taggedPostsByUser = /* @__PURE__ */ new Map();
		...
		const resolved = data.users.find((user) => user.id === actor || user.username === actor);
		viewerRow = resolved ?? null;
		db.liked = new Set(data.likes.filter((like) => like.user.id === resolved?.id).map((like) => like.post.id));
		db.saved = new Set(data.saves.filter((save) => save.user.id === resolved?.id).map((save) => save.post.id));
	}
	/**
	* @param {string} id
	* @returns {UserRow}
	...
	*/
	function media(post) {
		if (post.media_kind === "carousel") return { carousel: { slides: (db.slidesByPost.get(post.id) ?? []).map((slide) => ({
			id: slide.id,
			src: slide.file,
			alt: slide.alt
		})) } };
		if (post.media_file === null || post.media_alt === null) throw new Error(`post \`${post.id}\` has incomplete ${post.media_kind} media`);
		const ref = {
			src: post.media_file,
			alt: post.media_alt
		};
		if (post.media_kind === "video") {
			if (post.video_file === null) throw new Error(`video post \`${post.id}\` has no playable video_file`);
			return { video: {
				src: post.video_file,
				poster: ref
			} };
	...
	*/
	function postThumb(post) {
		...
		if (post.media_file === null || post.media_alt === null) throw new Error(`post \`${post.id}\` has no thumbnail media`);
		return {
			id: post.id,
			src: post.media_file,
			alt: post.media_alt
	...
	*/
	function storyRingsValue() {
		const rings = [...groupBy(db.stories.filter((story) => isHomeAuthor(story.author)), (story) => story.author).entries()].map(([author, stories]) => {
			stories.sort((left, right) => left.position - right.position || left.id.localeCompare(right.id));
			const unseen = stories.filter((story) => !db.storyViews.has(edgeKey(viewerId(), story.id)));
			...
			return {
				author,
				selected,
				newest: stories.reduce((latest, story) => newestFirst(latest.published_at, story.published_at) <= 0 ? latest : story),
				hasUnseen: !isSelf && unseen.length > 0
			};
		});
		rings.sort((left, right) => {
			const leftSelf = left.author === viewerId() ? 1 : 0;
			return (right.author === viewerId() ? 1 : 0) - leftSelf || newestFirst(left.newest.published_at, right.newest.published_at) || requireUser(left.author).username.localeCompare(requireUser(right.author).username);
		});
	...
	* @returns {Record<string, unknown>}
	*/
	function threadValue(postId) {
		return { comments: (db.commentsByPost.get(postId) ?? []).map((comment) => ({
			id: comment.id,
	...
	*/
	function storyValue(storyId) {
		...
		const sequence = db.stories.filter((candidate) => candidate.author === story.author).sort((left, right) => left.position - right.position || left.id.localeCompare(right.id));
		...
		return {
			...
			next: index + 1 < sequence.length ? sequence[index + 1]?.id ?? null : null,
	...
	/** @returns {Record<string, unknown>} */
	function reelsValue() {
		return { posts: db.posts.filter((post) => post.media_kind === "video").map((post) => postSummary(post)) };
	}
	/**
	* @param {string} userId
	* @returns {Record<string, unknown>}
	*/
	function profileValue(userId) {
		...
		const saved = userId === viewerId() ? db.posts.filter((post) => db.saved.has(post.id)) : [];
	...
	* @returns {Record<string, unknown>}
	*/
	function connectionsValue(userIds) {
		return { people: userIds.map((id) => requireUser(id)).sort((left, right) => left.username.localeCompare(right.username)).map((user) => ({
			user: userRef(user),
	...
	* @returns {Record<string, unknown>}
	*/
	function followersValue(userId) {
		requireUser(userId);
	...
	* @returns {Record<string, unknown>}
	*/
	function followingValue(userId) {
		requireUser(userId);
	...
	*/
	function searchValue(query) {
		...
		const people = [...db.users.values()].filter((user) => user.id !== viewerId()).filter((user) => needle.length === 0 || user.username.toLocaleLowerCase().includes(needle) || user.display_name.toLocaleLowerCase().includes(needle)).map((user) => user.id);
		const posts = db.posts.filter((post) => {
			if (needle.length === 0) return true;
			const author = requireUser(post.author);
			return post.caption.toLocaleLowerCase().includes(needle) || author.username.toLocaleLowerCase().includes(needle) || author.display_name.toLocaleLowerCase().includes(needle);
		});
		return {
	...
	* @returns {ProjectionUpdate[]}
	*/
	function postSettlementUpdates(postId, includeThread) {
		const updates = [
			projectionUpdate("feed", "feed-page", null, feedPageValue()),
			projectionUpdate("feed", "post-by-id", postId, postSummary(requirePost(postId))),
			projectionUpdate("feed", "reels", null, reelsValue())
		];
		if (includeThread) updates.push(projectionUpdate("comments", "for-post", postId, threadValue(postId)));
		return updates;
	}
	/**
	...
	*/
	function saveSettlementUpdates(postId) {
		return [...postSettlementUpdates(postId, false), projectionUpdate("profile", "profile", viewerId(), profileValue(viewerId()))];
	}
	/**
	...
	*/
	function storySettlementUpdates(storyId) {
		...
		return [projectionUpdate("feed", "feed-page", null, feedPageValue()), ...db.stories.filter((story) => story.author === author).map((story) => projectionUpdate("feed", "story-by-id", story.id, storyValue(story.id)))];
	}
	/** @returns {ProjectionUpdate[]} */
	function allSocialUpdates() {
		const updates = [projectionUpdate("feed", "feed-page", null, feedPageValue())];
		for (const userId of db.users.keys()) updates.push(projectionUpdate("profile", "profile", userId, profileValue(userId)), projectionUpdate("profile", "followers", userId, followersValue(userId)), projectionUpdate("profile", "following", userId, followingValue(userId)));
		updates.push(projectionUpdate("profile", "search-results", null, searchValue(searchQuery)));
		return updates;
	}
	/**
	...
	*/
	function fallbackUploadAlt(image) {
		...
		return fileName ? `Uploaded image “${fileName}” by ${author.display_name}` : `Image uploaded by ${author.display_name}`;
	}
	/**
	...
	* @returns {string}
	*/
	function payloadString(command, field) {
		const value = command.payload[field];
		if (typeof value !== "string") throw new Error(`command \`${command.port}/${command.command}\` needs string \`${field}\``);
		return value;
	}
	/**
	* @param {string} route
	...
	*/
	function refuseOrUnavailable(route, error) {
		const refusal = toRefusalName(error.code ?? "");
		if ((COMMAND_REFUSALS[route] ?? []).includes(refusal)) return { refused: { refusal } };
		return { unavailable: { reason: error.message ?? error.code ?? "provider error" } };
	}
	/**
	* @param {ProviderCommand} command
	...
	* @returns {void}
	*/
	function outcome(command, result, updates = []) {
		outbox.push(encode({
			kind: "outcome",
	...
	* @returns {Promise<void>}
	*/
	async function handle(command, pickedFile) {
		...
		try {
			switch (route) {
				...
				case "feed/unlike-post": {
					...
					const reply = await rpc(command.command === "like-post" ? "like_post" : "unlike_post", { post });
					if (reply.ok === false) {
				...
				case "feed/unsave-post": {
					const post = payloadString(command, "post");
					const reply = await rpc(command.command === "save-post" ? "save_post" : "unsave_post", { post });
					if (reply.ok === false) {
						outcome(command, refuseOrUnavailable(route, reply.error));
						return;
					}
				...
				case "comments/add-comment": {
					...
					const reply = await rpc("add_comment", {
						post,
						body: payloadString(command, "body")
					});
					if (reply.ok === false) {
						outcome(command, refuseOrUnavailable(route, reply.error));
						return;
				...
				case "feed/load-next-page":
					...
					outcome(command, { ok: {} }, [projectionUpdate("feed", "feed-page", null, feedPageValue())]);
					return;
				case "feed/reload":
					feedCount = PAGE_SIZE;
					await loadAll();
				...
				case "profile/unfollow-user": {
					...
					const reply = await rpc(command.command === "follow-user" ? "follow_user" : "unfollow_user", { target: user });
					if (reply.ok === false) {
						outcome(command, refuseOrUnavailable(route, reply.error));
						return;
					}
				...
				case "profile/search-people":
					...
					await loadAll();
					outcome(command, { ok: {} }, [projectionUpdate("profile", "search-results", null, searchValue(searchQuery))]);
					return;
				case "create/choose-image": {
					if (!pickedFile) throw new Error("this play host cannot choose local files");
					...
					if (!SUPPORTED_IMAGE_TYPES.has(picked.file.type.trim().toLowerCase())) {
						outcome(command, { refused: { refusal: "unsupported-media-type" } });
						return;
					}
					const object = await uploadFile(picked.file);
					uploadedFileNames.set(object, picked.file.name);
					outcome(command, { ok: {} }, [projectionUpdate("create", "draft", null, { uploaded: {
						object,
						preview: object,
				...
				case "create/publish-image": {
					...
					const reply = await rpc("create_image_post", {
						image,
						caption,
						alt: requestedAlt.trim().length > 0 ? requestedAlt : fallbackUploadAlt(image)
					});
					if (reply.ok === false) {
						outcome(command, refuseOrUnavailable(route, reply.error));
						return;
					}
					if (typeof reply.result !== "object" || reply.result === null || !("id" in reply.result) || typeof reply.result.id !== "string") throw new Error("create_image_post returned no post id");
					const post = reply.result.id;
					await loadAll();
					uploadedFileNames.delete(image);
					outcome(command, { ok: {} }, [
						projectionUpdate("feed", "feed-page", null, feedPageValue()),
						projectionUpdate("feed", "post-by-id", post, postSummary(requirePost(post))),
				...
				default: outcome(command, { unavailable: { reason: `no binding for command \`${route}\`` } });
			}
		} catch (error) {
	...
	return {
		systemInfo() {
			return {
				...
				actors: [...db.users.values()].sort((left, right) => left.username.localeCompare(right.username)).map((user) => ({
					id: user.id,
					username: user.username,
		...
		async assembleBoot() {
			...
			for (const post of db.posts) outbox.push(projectionMsg("comments", "for-post", post.id, threadValue(post.id)), projectionMsg("feed", "post-by-id", post.id, postSummary(post)));
			for (const story of db.stories) outbox.push(projectionMsg("feed", "story-by-id", story.id, storyValue(story.id)));
			outbox.push(projectionMsg("feed", "reels", null, reelsValue()));
			for (const userId of db.users.keys()) outbox.push(projectionMsg("profile", "profile", userId, profileValue(userId)), projectionMsg("profile", "followers", userId, followersValue(userId)), projectionMsg("profile", "following", userId, followingValue(userId)));
			outbox.push(projectionMsg("profile", "search-results", null, searchValue(searchQuery)));
		...
		deliver(commandJson) {
			...
			let pickedFile;
			if (`${command.port}/${command.command}` === "create/choose-image") try {
				pickedFile = host.pickFile({ accept: "image/jpeg,image/png,image/webp" }).then((file) => ({ file }), (error) => ({ error }));
			} catch (error) {
				pickedFile = Promise.resolve({ error });
			}
			inflight += 1;
			commandTail = commandTail.then(() => handle(command, pickedFile)).finally(() => {
				inflight -= 1;
			});
		},
		tick() {
