...
import type { ResolveAsset } from "./assets.js";
...
window.__uhura = runtime;

// Keep the Wasm package as a runtime-owned absolute module. The ignore marker
// is required in both Vite dev and production: `/wasm/` is served/proxied by
// the Uhura host and must never be resolved into the Play bundle. Install the
// stable runtime handle before this first await so Restart remains available
// even when loading Wasm fails.
const wasmModuleUrl = "/wasm/uhura_wasm.js";
let wasmModule: typeof import("/wasm/uhura_wasm.js");
try {
  wasmModule = await import(/* @vite-ignore */ wasmModuleUrl);
} catch (error) {
  systemControls.failed(error);
  overlay.showFatal(`failed to load the Wasm runtime: ${String(error)}`);
  throw error;
}
const { default: init, FixtureDriver, Session, protocols } = wasmModule;
...
async function importProvider(module: string): Promise<ProviderModule> {
  try {
    ...
    if (sessionStorage.getItem("uh-provider-retry") === null) {
      ...
      location.reload();
      await new Promise<never>(() => {}); // reloading — never resolve
    }
    throw error;
  }
}
...
async function fetchArtifacts<const T extends readonly string[]>(
  ...
  return texts as { [K in keyof T]: string };
}
...
async function boot(): Promise<void> {
  ...
  const glyphs = JSON.parse(iconsText) as Record<string, string>;
  ...
  let nextNavToken = 1;
  const navFrames: { params: string; token: number }[] = [{ params: "{}", token: 0 }];
