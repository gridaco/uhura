 ...
 *   onStep: (result: import("./types.js").StepResult) => void,
 *   onError: (error: unknown, eventJson: string) => void,
 *   onDrained?: () => void,
 * }} wiring
 */
export function createPump({ dispatch, deliver, onStep, onError, onDrained }) {
  /** @type {{ eventJson: string, onApplied?: () => void }[]} */
  const queue = [];
  let pumping = false;
   ...
   */
  function enqueue(eventJson, onApplied) {
    queue.push({ eventJson, onApplied });
    pump();
  }
  ...
  function pump() {
    ...
    try {
      while (queue.length > 0) {
        ...
        /** @type {import("./types.js").StepResult} */
 ...
 */
export function providerMsgToEvent(msgJson) {
  /** @type {import("./types.js").ProviderMsg} */
