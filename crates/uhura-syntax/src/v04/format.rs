...
pub enum FormatError {
    /// RFC 0003 comments were found, but their checked attachment is not yet
    /// represented in the 0.4 AST.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnsupportedComments { comments } => write!(
                formatter,
                "cannot canonically format {} comment(s) before 0.4 comment attachment is available",
                comments.len()
            ),
        }
    }
...
impl Formatter {
    ...
    fn ui_node(&mut self, node: &UiNode) {
        match &node.kind {
            ...
            UiNodeKind::Comment(comment) => {
                self.write("<!--");
                self.write(&normalize_ui_comment(&comment.body));
                self.write("-->");
            }
...
fn normalize_ui_comment(value: &str) -> String {
