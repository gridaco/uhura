//! Serialization-friendly, source-spanned syntax for the current Uhura language.
//!
...
pub struct UiComment {
    ...
    pub body: String,
    /// Lexical/attachment state retained so formatting a recovered tree cannot
    /// turn invalid or rejected source into newly attachable metadata.
    pub status: UiCommentStatus,
    /// A valid leading `@kind` marker, normalized for checked authoring
    /// metadata. Ordinary comments leave this empty.
    pub annotation: Option<MarkupAnnotation>,
}

#[derive(Serialize, Deserialize, Clone, Copy, Debug, PartialEq, Eq)]
pub enum UiCommentStatus {
    Ordinary,
    Annotation,
    Malformed { terminated: bool },
    RejectedAnnotation,
}
...
pub struct MarkupAnnotation {
    pub kind: String,
    pub text: String,
    /// Full source span of the carrying `<!-- ... -->` comment.
    pub span: Span,
}
...
pub struct UiElement {
    ...
    /// Forward-attached authoring annotations in source order.
...
pub struct UiIf {
    ...
    /// Annotations target the complete authored block, not one lowered arm.
    pub annotations: Vec<MarkupAnnotation>,
    pub open_span: Span,
...
pub struct UiEach {
    ...
    pub key: Expression,
    pub children: Vec<UiNode>,
    /// Annotations target the complete keyed block.
    pub annotations: Vec<MarkupAnnotation>,
    pub open_span: Span,
    pub close_span: Span,
}
