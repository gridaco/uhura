//! Pure, canonical Uhura 0.4 project compilation.
//!
...
pub struct V04ProjectSource<'a> {
    pub file: FileId,
    pub path: &'a str,
    pub text: &'a str,
}

impl<'a> V04ProjectSource<'a> {
...
pub fn compile_v04_project(
    manifest: &ProjectManifest,
    sources: &[V04ProjectSource<'_>],
    ...
    let root_modules = manifest
        ...
        .collect();
    packages.push(crate::v04::CapturedPackageModules {
    ...
    let mut output =
        crate::v04::check_package_graph_with_evidence(&root_package, &packages, &evidence_modules);
...
fn index_sources<'a>(
    sources: &'a [V04ProjectSource<'a>],
) -> (BTreeMap<&'a str, &'a V04ProjectSource<'a>>, Vec<Diagnostic>) {
...
fn required_source<'a>(
    sources: &BTreeMap<&'a str, &'a V04ProjectSource<'a>>,
    path: &str,
    missing: String,
    diagnostics: &mut Vec<Diagnostic>,
) -> Option<&'a V04ProjectSource<'a>> {
...
fn parse_core_module(
    source: &V04ProjectSource<'_>,
    package: &str,
    logical: &str,
    context: Option<String>,
    diagnostics: &mut Vec<Diagnostic>,
) -> v04::Module {
    let parsed = v04::parse(
        v04::SourceIdentity::new(source.file.0, package, logical, source.path),
        source.text,
    );
...
fn source_span(source: &V04ProjectSource<'_>) -> Span {
    Span::new(
        source.file,
        0,
