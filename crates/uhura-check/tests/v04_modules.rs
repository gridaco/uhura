...
use uhura_check::{check_v04_project_modules, resolve_v04_project_modules};
...
fn private_import_is_rejected_at_the_authored_locator() {
    ...
    let output =
        check_v04_project_modules(&[module(41, "support", "support.uhura", &support), imported]);
...
fn package_global_public_name_collision_is_rejected() {
    ...
    let output = check_v04_project_modules(&[second, first]);
...
fn uncomposed_parts_and_known_standard_features_are_inert() {
    ...
    let output = check_v04_project_modules(&[part, standard]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics
...
fn external_alias_requires_a_resolved_package_graph() {
    ...
    let output = check_v04_project_modules(&[external]);
    assert!(output.program.is_none());
...
"#,
    ...
    let output = check_v04_project_modules(std::slice::from_ref(&port));
...
"#,
    ...
    let output = check_v04_project_modules(&[support, machine]);
    assert!(output.program.is_none());
    assert!(
        output
            .diagnostics
...
fn lexical_bindings_may_shadow_an_unimported_package_name() {
    ...
    let output = check_v04_project_modules(&[support, consumer]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics
...
"#,
    ...
    let output = check_v04_project_modules(&[left, right]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics,
...
"#,
    ...
    let output = check_v04_project_modules(&[source]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics,
...
"#;
    let output = check_v04_project_modules(&[module(1, "program", "program.uhura", source_text)]);
