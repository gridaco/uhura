//! Canonical provenance behavior.

use std::collections::{BTreeMap, BTreeSet};

use uhura_check::{build_provenance, check_project_modules};
use uhura_core::{
...
fn composed_topology_survives_as_a_source_sidecar_and_merges_into_one_runtime_graph() {
    ...
    let provenance = build_provenance(&modules).expect("validated authored topology");
...
fn covers_public_declarations_machine_members_ui_imports_and_direct_parts() {
    ...
    let provenance = build_provenance(&modules).expect("validated provenance");
    provenance.validate().expect("closed provenance contract");
...
fn file_module_comment_and_format_moves_preserve_nodes_but_change_provenance_revision() {
    ...
    let before = build_provenance(&before_modules).expect("before provenance");
    let after = build_provenance(&after_modules).expect("after provenance");
...
fn rejects_out_of_bounds_and_non_utf8_boundary_occurrence_spans() {
    ...
    let error = build_provenance(&[outside]).expect_err("range must be rejected");
    ...
    let error = build_provenance(&[split]).expect_err("UTF-8 split must be rejected");
