...
use uhura_syntax::{ParseDiagnosticKind, SourceIdentity, parse};

use std::fmt::Write as _;

fn module(source: &str) -> uhura_syntax::ast::Module {
    let parsed = parse(
...
fn checked(source: &str) -> uhura_core::Program {
    let output = check_project_modules(&[module(source)]);
    assert!(
...
fn checked_program_size(source: &str) -> usize {
    ...
    let output = check_project_modules(&[parsed]);
    assert!(
        output.diagnostics.is_empty(),
        "check diagnostics:\n{:#?}",
        output.diagnostics
...
"#;
    ...
    let invalid = check_project_modules(&[module(
        r#"
pub machine Invalid {
  events { Run }
  state {}
...
fn valueless_return_checks_as_unit_and_return_outside_a_body_is_rejected() {
    let valueless = check_project_modules(&[module(
        r#"
pub fn invalid() -> Int {
  return;
}
"#,
    ...
    let outside = check_project_modules(&[module(
        r#"
