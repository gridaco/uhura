...
fn module(file: u32, logical: &str, source: &str) -> uhura_syntax::ast::Module {
    let parsed = parse(
...
fn checked(sources: &[uhura_syntax::ast::Module]) -> uhura_core::Program {
    let output = check_project_modules(sources);
    assert!(
...
fn diagnostic_rules(source: &str) -> Vec<String> {
    let output = check_project_modules(&[module(1, "invalid", source)]);
...
"#;
    ...
    let provenance = build_provenance(std::slice::from_ref(&original)).unwrap();
    for (node, owner) in [
        (&left_invariant, "left"),
