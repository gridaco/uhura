...
use uhura_check::{
    V04CapturedPackageModules, V04ProjectSource, check_v04_package_graph_with_evidence,
    compile_v04_project,
};
...
fn canonical_service_matches_the_explicit_parse_graph_check_pipeline() {
    ...
    let sources = [
        V04ProjectSource::new(FileId(0), "counter.uhura", ROOT_SOURCE),
        V04ProjectSource::new(FileId(1), "counter.evidence.uhura", EVIDENCE_SOURCE),
        V04ProjectSource::new(FileId(2), "vendor/shared/values.uhura", DEPENDENCY_SOURCE),
    ];

    let canonical =
        compile_v04_project(&root_manifest, &sources, std::slice::from_ref(&dependency));
    assert!(
    ...
    let root = v04::parse(
        v04::SourceIdentity::new(0, "test.counter@1", "counter", "counter.uhura"),
        ROOT_SOURCE,
    );
    let shared = v04::parse(
        v04::SourceIdentity::new(2, "test.shared@1", "values", "vendor/shared/values.uhura"),
        DEPENDENCY_SOURCE,
    );
    assert!(root.diagnostics.is_empty());
    assert!(shared.diagnostics.is_empty());
    let evidence = v04::parse(
        v04::SourceIdentity::new(1, "test.counter@1", "evidence", "counter.evidence.uhura"),
        EVIDENCE_SOURCE,
    );
    assert!(evidence.diagnostics.is_empty());
    let explicit = check_v04_package_graph_with_evidence(
        "test.counter@1",
        &[
...
fn diagnostic_order_is_independent_of_source_inventory_order() {
    ...
    let ordered = [
        V04ProjectSource::new(FileId(3), "counter.uhura", bad_root),
        V04ProjectSource::new(FileId(1), "vendor/shared/values.uhura", bad_dependency),
    ];
    let reversed = [ordered[1], ordered[0]];

    let first = compile_v04_project(&root_manifest, &ordered, std::slice::from_ref(&dependency));
    let second = compile_v04_project(&root_manifest, &reversed, std::slice::from_ref(&dependency));
