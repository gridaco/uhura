use uhura_check::{V04CapturedPackageModules, check_v04_package_graph_with_evidence};
...
fn graph(root_alias: &str, middle_alias: &str) -> Vec<V04CapturedPackageModules> {
...
fn transitive_external_imports_link_by_exact_package_identity() {
    let output =
        check_v04_package_graph_with_evidence("example.root@1", &graph("kit", "base"), &[]);
    assert!(
...
fn dependency_aliases_are_nonsemantic() {
    ...
    let renamed = check_v04_package_graph_with_evidence(
        "example.root@1",
        &graph("toolkit", "foundation"),
        &[],
    )
    .program
...
fn unused_dependency_declarations_do_not_enter_machine_program_identity() {
    ...
    let ordinary = check_v04_package_graph_with_evidence("example.root@1", &ordinary_graph, &[])
        .program
        .expect("ordinary graph");
    let changed = check_v04_package_graph_with_evidence("example.root@1", &changed_graph, &[])
        .program
...
"#,
    ...
    let direct =
        check_v04_package_graph_with_evidence("example.root@1", &[direct_root, base.clone()], &[])
            .program
            .expect("direct import graph");
    let reexported = check_v04_package_graph_with_evidence(
        "example.root@1",
        &[reexport_root, facade, base],
        &[],
    )
    .program
...
fn external_reexport_cannot_replace_a_package_public_name() {
    ...
    let output =
        check_v04_package_graph_with_evidence("example.root@1", &[root, facade, base], &[]);
...
"#,
    ...
    let output = check_v04_package_graph_with_evidence("example.root@1", &[root, left, right], &[]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics
...
fn undeclared_alias_and_private_external_name_are_rejected() {
    ...
    let output = check_v04_package_graph_with_evidence("example.root@1", &[root, private], &[]);
    assert!(output.program.is_none());
    let rules = output
        .diagnostics
        .iter()
...
"#,
    ...
    let output = check_v04_package_graph_with_evidence("example.root@1", &[root, dependency], &[]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics
...
fn external_part_public_id_is_machine_identity_material_but_its_locator_is_not() {
    let ordinary = check_v04_package_graph_with_evidence(
        "example.root@1",
    ...
    let relocated = check_v04_package_graph_with_evidence(
        "example.root@1",
    ...
    let different_provider = check_v04_package_graph_with_evidence(
        "example.root@1",
...
fn external_part_reexport_preserves_the_provider_public_id() {
    let direct = check_v04_package_graph_with_evidence(
        "example.root@1",
        &external_counter_graph("vendor", "vendor.parts@1", "parts"),
        &[],
    )
...
"#,
    ...
    let reexported =
        check_v04_package_graph_with_evidence("example.root@1", &[root, facade, provider], &[])
            .program
...
"#,
    ...
    let output = check_v04_package_graph_with_evidence("example.root@1", &[root, parts, base], &[]);
    assert!(
        output.diagnostics.is_empty(),
        "diagnostics:\n{:#?}",
        output.diagnostics
...
"#,
    ...
    let output =
        check_v04_package_graph_with_evidence("example.root@1", &[root, private, left, right], &[]);
    assert!(output.program.is_none());
    let rules = output
        .diagnostics
        .iter()
