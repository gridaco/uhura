//! Coherent Uhura project capture shared by every embedding surface.
//!
//! A consumer observes one immutable set of project-relative bytes and gives
...
/// Normalize one project-relative path without consulting the filesystem.
///
/// This is also used by host resource admission so source and resource paths
/// share one escape policy.
#[doc(hidden)]
...
impl ProjectSourceSnapshot {
    ...
    /// Read one UTF-8 file from this exact captured revision.
    #[doc(hidden)]
    pub fn read_text(&self, relative: &str) -> Result<Option<String>, String> {
        self.files.text(relative)
    }

    /// Read one file from this exact captured revision.
    #[doc(hidden)]
    pub fn read_bytes(&self, relative: &Path) -> Result<Option<Arc<[u8]>>, String> {
        self.files.resolve(relative).map(|bytes| bytes.cloned())
    }

    /// Project all captured files below `prefix`, keyed relative to it.
    #[doc(hidden)]
    pub fn subtree(&self, prefix: &Path) -> BTreeMap<String, Arc<[u8]>> {
        self.files.subtree(prefix)
    }

    pub(crate) fn capture_failure_messages(&self) -> Vec<String> {
...
mod tests {
    ...
    fn capture_ignores_generated_roots_and_keeps_uhura_sources() {
        ...
        assert!(snapshot.capture_failure_messages().is_empty());
